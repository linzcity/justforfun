...
def main():
    ...
    #print('通讯录共%s位好友' % len(MemberList),file=doc)

    for x in MemberList:
		# 这里只将签名写进文件，可按需写入各种数据，后续生成词云是根据这些数据生成
        #sex = '未知' if x['Sex'] == 0 else '男' if x['Sex'] == 1 else '女'
        #print('昵称:%s, 性别:%s, 备注:%s, 签名:%s<br><br>' % (x['NickName'], sex, x['RemarkName'], x['Signature']),file=doc)
        if 'span' in x['Signature']:
            continue
        print('%s\n'% x['Signature'],file=doc)
