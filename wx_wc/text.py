...
def wordcloudplot(txt):
    path = '../jieba/msyh.ttc' # 字体路径 ，路径都为绝对路径
    # path = unicode(path, 'utf8').encode('gb18030')
    alice_mask = np.array(PIL.Image.open('../jieba/背景图片库/qishi.jpeg')) # 背景图片路径，注意只有不透明部分会填充
    ...
    wordcloud.to_file('./output.jpg') # 生成图片位置：当前目录下
...
def main():
    a = []
    f = open('output.txt', 'rb').read() # 文档位置：当前目录下
